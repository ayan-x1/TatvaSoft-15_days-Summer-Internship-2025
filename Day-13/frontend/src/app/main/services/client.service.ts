...
export class ClientService {
  ...
  getUserProfileDetailById(userId: any) {
    ...
    return this.http.post<any>(`${this.apiUrl}${API_ENDPOINTS.COMMON.UPLOAD_IMAGE}`, formData);
  }

  updateUserProfile(formData: FormData): Observable<any> {
    return this.http.put<any>(`${this.apiUrl}${API_ENDPOINTS.AUTH.UPDATE_USER}`, formData);
  }

  updateUserProfileImage(data: any): Observable<any> {
    return this.http.post<any>(`${this.apiUrl}${API_ENDPOINTS.AUTH.UPDATE_USER_PROFILE}`, data);
  }
