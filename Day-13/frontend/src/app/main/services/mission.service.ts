...
missionApplicationDelete(data: any) {
  // Extract the application ID from the data object
  const applicationId = data.id || data.applicationId || data.Id;
  
  // Send the raw ID as the request body (backend expects [FromBody] int applicationId)
  return this.http.post(`${this.apiUrl}${API_ENDPOINTS.MISSION.APPLICATION_DELETE}`, applicationId);
}
